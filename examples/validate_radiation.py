...
Runs a moist Byrne aquaplanet to quasi-equilibrium, then computes the
full SW and LW radiation budget at every grid point.  Reports global-mean
and zonal-mean diagnostics to verify:
...
Usage
-----
    uv run python examples/validate_radiation.py
    uv run python examples/validate_radiation.py --days 400 --spinup 200
"""
...
def compute_olr(
    ...
    """Compute outgoing longwave radiation (upward LW flux at TOA) [W/m²].

    Performs only the upward scan from surface to TOA (half the cost
    of ``longwave_heating``).
    ...
    tau_half : jnp.ndarray
        LW optical depth at half-levels, shape
        ``(n_levels+1, n_lat)`` or ``(n_levels+1, n_lat, n_lon)``.

    Returns
    -------
    jnp.ndarray
        OLR [W/m²], shape ``(n_lat, n_lon)``.
    """
    ...
    # Upward scan from surface (reversed)
...
def diagnose_radiation_budget(
    ...
    Returns a dictionary of 2-D fields (n_lat, n_lon).
    """
    ...
    # --- LW optical depth ---
    if cfg.radiation_scheme == "byrne" and q_grid is not None:
        ...
        msg = "Validation requires Byrne scheme with humidity"
    ...
    # --- SW without humidity (for comparison) ---
    _, sw_down_sfc_dry = shortwave_heating(
        levels.sigma_half,
        levels.dsigma,
        sin_lat,
        surface_pressure,
    ...
    # Atmospheric SW absorption (column integral of heating * dp / g)
    ...
    # LW atmospheric cooling (should be negative — atmosphere loses energy)
    lw_atm_cooling = jnp.sum(
        lw_heating * planet.specific_heat_cp * dp / planet.gravity,
        axis=0,
    )

    # Surface SW absorbed (after albedo)
    sw_sfc_absorbed = sw_down_sfc * (1.0 - planet.surface_albedo)

    # SW reflected to space: what's left after atmosphere + surface absorb
    sw_reflected_space = toa_sw_in - sw_atm_absorbed - sw_sfc_absorbed

    # --- Surface energy budget ---
    # Sensible + latent heat fluxes (approximate from lowest-level state)
    ...
    # Net surface flux (positive = warming surface)
    net_sfc = sw_sfc_absorbed + lw_down_sfc - lw_up_sfc - sensible - latent

    # Net TOA flux (positive = planet gaining energy)
    net_toa = (toa_sw_in - sw_reflected_space) - olr

    # Column closure: atm_absorbed + sfc_absorbed + reflected = TOA_in
    ...
    return {
        ...
        "sw_down_sfc_dry": sw_down_sfc_dry,
...
def run_validation(
    ...
    """Run Byrne aquaplanet and validate radiation budget."""
    ...
    print("  Radiation: Byrne LW + humidity-dependent SW (tau=0.22)")
    ...
    print(f"  Day 1 (incl. compile): {t_compile:.1f}s")

    # --- Integration ---
    # Accumulate diagnostic budget over the averaging period
    n_samples = 0
    ...
    for day in range(2, n_days + 1):
        ...
        # Averaging period: accumulate radiation budget
        if day > spinup_days:
...
def _print_report(
    mean_budget: dict[str, jnp.ndarray],
    grid: GaussianGrid,
) -> bool:
