#!/usr/bin/env python3
"""Moist aquaplanet integration (Frierson 2006/2007).

Runs a moist aquaplanet with gray longwave radiation, simplified
Betts-Miller convection, large-scale condensation, bulk surface
fluxes (sensible + latent heat), and Rayleigh boundary-layer drag.
Starts from an isothermal rest state with an RH-based humidity profile.

After a spinup period, accumulates time-averaged zonal-mean fields and
prints basic diagnostics including moisture and precipitation.

Usage
-----
    uv run python examples/moist_aquaplanet.py
    uv run python examples/moist_aquaplanet.py --days 300 --spinup 100
    uv run python examples/moist_aquaplanet.py --truncation 42 --dt 600
"""
...
def run_moist_aquaplanet(
    ...
    """
    print(f"Moist aquaplanet: T{truncation} L{n_levels}, dt={dt:.0f}s, {n_days} days")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Moist Frierson aquaplanet")
